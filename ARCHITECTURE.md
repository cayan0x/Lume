...
| 测试          | `npm test`（条数以 `npm test` 当场输出为准）                                        | 判据行为 + client 纯逻辑；**宿主形状用 `test/fixtures/host-events/*.json` 的真机样本**；另有 `scripts/mechanism-coverage.mjs` 要求每个机制各有行为测试（条数以 `npm run lint` 的机制覆盖输出为准） |
