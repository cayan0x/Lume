...
- **feat** 导出弹窗 UI 整洁化：此前点某行「导出」会在该行原地展开成「包含记忆 + 取消 + 确认导出」一排控件，把列表行挤兑得难以阅读。现改为在管理弹窗顶部弹出独立的确认条块（显示人设名 + 包含记忆复选框 + 取消 / 确认导出），列表行不再被挤压，视觉与弹窗整体风格统一
- **docs** README 重构：更换入设菜单 / 管理 / 编辑 / 蒸馏 / 记忆星图五张新截图，新增各章节配图与描述；补充「切换时机提醒」——在已对话的会话中切换人设需明确告诉大模型，单纯点击菜单会因思维惯性不即时生效
