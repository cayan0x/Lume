...
export function buildCorpusPrompt(input) {
    ...
    return {
        system: [
            ...
            "- assistant：角色的回应。优先直接复用素材原句（只做让对话成立的最小改写），完整保留素材中的称呼、口头禅、语气词和语气强度——禁止把强烈的语气中和成平淡的通用回复；第一句就入戏，每条 ≤240 字。",
...
async function callJson(deps, route, system, userText, maxTokens) {
...
export async function runDistill(deps, input, onProgress) {
    const text = input.text.trim();
    if (!text)
        throw new Error("distill: 素材为空");
    if (text.length > DISTILL_TEXT_CAP)
    ...
    if (contractOut === null)
        throw new Error("distill: 契约合成失败（模型输出无法解析）");
...
export class DistillJobRunner {
    ...
    start(input) {
        ...
        if (text.length > DISTILL_TEXT_CAP)
            throw new Error(`素材超过 ${DISTILL_TEXT_CAP} 字上限`);
        this.#sweep();
        const id = `distill-${Date.now().toString(36)}-${++jobSeq}`;
        const job = { id, status: "running", at: Date.now(), stage: "mining" };
