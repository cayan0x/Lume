...
export function carrierBlocks(deps, input) {
    ...
    if (!noticeText(st, "coverage") && noticeOpen(st, "coverage") && st.agent.artifactText.length > 200) {
        ...
        if (items.length >= 2) {
            setNotice(st, "coverage", deps.buildRequirementCoverageDirective(deps.coverageRows(items, st.agent.artifactText), {
    ...
    return [
        ...
        // 所以新会话必须能一眼看到上次的目标、已拍板、未决与关键定位。
        {
            text: deps.isColdStart({ hasContract: Boolean(deps.contractOf(sid)), changes: deps.changesOf(sid).length, requirements: deps.requirementsOf(sid).length })
                ? deps.renderTaskMemory(st.taskMemories?.[0] ?? null, {
                    recent: (st.taskMemories ?? []).slice(0, 4).map((item) => ({ title: String(item.title ?? ""), at: Number(item.at ?? 0) })),
                })
                : null,
            droppable: true,
        },
