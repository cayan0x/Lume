        ...
        switch (event.type) {
            ...
            case "assistant/message": {
                ...
                if (text) {
                    ...
                    deps.setNotice(st, "citation", deps.buildCitationDirective(citations, (key) => deps.formatWindows(st.evidence, key)));
            ...
            case "tool/call": {
                ...
                if (st.toolKind === "inspect") {
                    if (/read|view|cat|open|head|tail/i.test(callName))
                        deps.recordReadArgs(st.evidence, callArgs);
            ...
            case "tool/result": {
                ...
                if (st.toolKind === "inspect" && resultText)
                    deps.recordResultText(st.evidence, resultText);
                // 所有工具结果都算「见过」：否定断言只能用见过的东西支撑
                if (resultText)
                    deps.recordSymbols(st.seenSymbols, resultText);
