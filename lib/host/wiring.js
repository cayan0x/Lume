 ...
 */
export function ensureSessionWorkspace(sid, st) {
    if (st.cwd)
        return;
    const home = resolveDsHome();
    if (!home)
...
export function rememberSessionWorkspace(sid, cwd) {
    const home = resolveDsHome();
    if (!home)
        return;
...
export function startSessionBackfill(input) {
    ...
    return startBackfill({
        dsHome,
        extract: (text, source, userText) => extractKnowledgeCandidates(text, { source, userText }).map((c) => ({ kind: c.kind, text: c.text })),
        messageText,
        visibleText,
