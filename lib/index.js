...
function applyInner(ctx, config = {}) {
        ...
        switch (event.type) {
            ...
            case "assistant/message": {
                ...
                if (text) {
                    ...
                    if (setNotice(st, "drift", buildDriftDirective(driftWords)))
            ...
            case "tool/call": {
                ...
                if (st.toolKind === "inspect" && toolTargetOf(event.data))
                ...
                if (st.toolKind === "inspect") {
                    if (/read|view|cat|open|head|tail/i.test(callName))
                        recordReadArgs(st.evidence, callArgs);
                    if (st.lastToolTarget)
                        st.inspectedTargets.add(st.lastToolTarget);
                }
                if (projectMemoryOn && st.toolKind === "mutate") {
                    ...
                    if (st.triggerCounters.mutations === 1 && target && !st.inspectedTargets.has(target)) {
                        ...
                        if (!noticeText(st, "trigger"))
                            forceNotice(st, "trigger", `〔先定位〕你要改 ${target}，但本会话还没有读过它——已经摸过的是：${seen}。先打开要改的那段（含调用方与配置/SQL 绑定），确认现有实现再动手；改完立刻回读或跑最小验证。`);
            ...
            case "tool/result": {
                ...
                if (st.toolKind === "inspect" && resultText)
                    recordResultText(st.evidence, resultText);
                // 验证结算：成功的真验证自动推进台账 / 失败立刻顶一句先修红——都不等模型调工具
                if (projectMemoryOn)
                    settleVerification(sid, st, resultText, signals);
            ...
            case "turn/end": {
                ...
                if (failed && queryKey && queryKey === st.lastFailureQuery)
                    ...
                    forceNotice(st, "protocol", null);
                }
                if (st.failureStreak >= 2)
                    forceNotice(st, "protocol", "检测到相同请求连续失败：先定位根因并记录已排除假设，再选择不同方案；不要重复同一调用。");
                ...
                forceNotice(st, "postTurn", [deliveryNotice, carrierGap].filter(Boolean).join("\n\n") ||
    ...
    ctx.effect(() => ctx.on("session/disposed", (session) => {
        ...
        if (st.pendingFacts.length > 0)
            ctx.logger?.warn?.(`lume: [${sid}] 项目知识未落盘（无法确定工作目录）：${st.pendingFacts.length} 条`);
        runtime.delete(sid);
