...
check(
	"能力行含新层（载具/触发器/设计 pass/需求锚点）",
	Boolean(capability && /载具/.test(capability) && /触发器/.test(capability)),
	capability ?? "能力行缺失",
);
...
check(
	"RPC 没有两路全挂（无 shapes: 诊断）",
	!rpcLines.some((l) => l.includes("shapes:")),
	"出现 shapes: 说明 registerRpcChannel 两路都失败",
);
...
const decode = (file) => {
	...
	for (let i = 0; i < idx.length; i++) {
		try {
			text += zstdDecompressSync(buf.subarray(idx[i], i + 1 < idx.length ? idx[i + 1] : buf.length)).toString("utf8");
		} catch {
			/* skip */
		}
	}
...
for (const slug of readdirSync(sessionsRoot)) {
	let subs = [];
	try {
		subs = readdirSync(join(sessionsRoot, slug));
	} catch {
		continue;
	}
	for (const sub of subs) {
		...
		try {
			...
			if (!newest || m > newest.m) newest = { file, slug, sub, m };
		} catch {
			/* none */
		}
	}
...
if (newest) {
	...
	const detail = Object.entries(blocks)
		.map(([name, re]) => `${name} ${(text.match(re) ?? []).length}`)
		.join(" · ");
...
try {
	...
	if (gens.length === 0) check("live 客户端产物可解析", false, "没找到 lib/client.js");
		...
		for (const g of gens) {
			try {
				new vm.Script(readFileSync(g.file, "utf8"));
			} catch {
				bad++;
			}
		}
		check(
			"live 客户端产物可解析（全部 " + gens.length + " 个）",
			bad === 0,
			bad === 0 ? "最新 " + gens[0].name.slice(0, 40) : bad + " 个解析失败",
		);
	}
} catch (error) {
