	...
	useEffect(() => {
		...
		const draw = () => {
			...
			for (const e of edges) {
				...
				ctx.moveTo(a.x, a.y);
				ctx.lineTo(b.x, b.y);
				ctx.strokeStyle = `rgba(90,170,255,${0.1 + e.weight * 0.2})`;
			...
			for (const e of relEdges) {
				...
				ctx.lineWidth = 1.4 + e.weight * 2.0;
				ctx.stroke();
				ctx.restore();
			}
			for (const n of nodes) {
		...
		>
			...
			>
				...
				>
					...
					<span style={{ fontSize: 15, fontWeight: 500, color: "#d4e2f8", letterSpacing: 1 }}>
						{personaLabel} · {t("memory.title")}
					</span>
					<span style={{ flex: 1 }} />
					{/* 日期筛选 */}
					...
					{selectedNode ? (
						...
						>
							<div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
								...
								/>
								<span style={{ fontSize: 11, color: "rgba(170,190,220,0.6)" }}>
									{selectedNode.core ? t("memory.core") : t("memory.plain")} · {relTime(selectedNode.at)}
								</span>
							</div>
							{editing ? (
								<textarea
