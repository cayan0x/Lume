 ...
 */
export function splitRequirementItems(text: unknown): RequirementItem[] {
	...
	for (const raw of lines) {
		...
		if (sub) {
			...
			if (sub[2]!.trim()) items.push({ index: items.length + 1, label, text: sub[2]!.trim() });
			continue;
		}
		const top = TOP_ITEM_RE.exec(line);
		if (top && !SECTION_RE.test(line)) {
			topLabel = top[1]!;
			if (top[2]!.trim()) items.push({ index: items.length + 1, label: topLabel, text: top[2]!.trim() });
			continue;
		}
		// 无编号的续行：并到上一条（原文里常见的换行折行）
		if (items.length > 0 && !SECTION_RE.test(line) && !/^[（(]?图/.test(line)) {
			const last = items[items.length - 1]!;
			if (last.text.length < 300) last.text = `${last.text}${line}`;
		}
	}
	return items;
}
...
export function ngrams(text: string, n: number): string[] {
	...
	for (let i = 0; i + n <= text.length; i++) {
		...
		if (GENERIC_GRAM_RE.test(gram)) continue; // 含虚词/标点/泛化词的片段不算
		if (new Set(gram).size < n) continue;
		out.push(gram);
	}
	return [...new Set(out)];
}
...
export interface CoverageMention {
	/** 交付物里的句子 */
	text: string;
	/** 与需求原文共享的最长片段（把片段也摆出来：只匹配到「业务类型」这种短片段，说明要点可能没写进去） */
	gram: string;
}
...
export interface CoverageRow {
	label: string;
	text: string;
	/** 交付物里提到这条的句子（最多 3 句，按命中片段长度降序） */
	mentions: CoverageMention[];
}
 ...
 */
export function coverageRows(items: readonly RequirementItem[], artifact: string): CoverageRow[] {
	const sentences = sentencesOf(artifact);
	return items.map((item) => {
		const gram3 = ngrams(item.text, 3);
		const gram4 = ngrams(item.text, 4);
		const scored: Array<{ text: string; gram: string; tier: number }> = [];
...
export function hasFigureRefs(text: unknown): boolean {
	return /图[一二三四五六七八九十\d]|如图|截图|附件/.test(String(text ?? ""));
}

/** 交付文案里引用的章节号是否真的存在于交付物里（落点编造/章节被删的机械判据）。 */
export function danglingSectionRefs(deliveryText: unknown, artifact: string): string[] {
	const headings = new Set<string>();
	const headingRe = /^#{1,6}\s*(\d{1,2}(?:\.\d{1,2}){0,2})/gm;
	let m: RegExpExecArray | null;
	while ((m = headingRe.exec(artifact)) !== null) headings.add(m[1]!);
	if (headings.size < 3) return []; // 交付物没有稳定编号体系 → 这条核对不成立
	const refs = new Set<string>();
	const refRe = /\b(\d{1,2}\.\d{1,2}(?:\.\d{1,2})?)\b/g;
	while ((m = refRe.exec(String(deliveryText ?? ""))) !== null) refs.add(m[1]!);
 ...
 */
export function pickRequirementCorpus(items: readonly { text: string }[]): string {
