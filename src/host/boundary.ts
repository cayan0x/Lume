/**
 * 人设切换播报组装（纯函数）：从 index.ts 抽出，与运行时状态无关。
 *
 * composeBoundary 原先内嵌在 apply() 闭包中，依赖 registry 和 labelOf 逻辑。
 * 这里把 registry 作为参数注入，保持纯函数可测。
 */
...
function labelOf(registry: PersonaRegistry, personaName: string | null | undefined): string {
 ...
 */
export function composeBoundary(input: BoundaryInput): string | null {
