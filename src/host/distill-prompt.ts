/**
 * 蒸馏的提示词与解析（从 distill.ts 拆出，架构整理 ⑤）。
 *
 * 这里全是**纯字符串工作**：拼提示词、宽松解析模型输出、归一成可存储的卡片。
 * 与 runner（distill.ts 的 runDistill / DistillJobRunner）分开的原因：这两类东西的
 * 变更节奏完全不同——提示词按效果迭代、runner 按调度需求改，混在一个 560 行文件里互相拖累。
 * 单测可以直接喂字符串，不需要模型或宿主。
 */
