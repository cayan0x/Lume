...
import type { CoverageRow } from "../core/coverage.js";
...
export function buildContractMethodDirective(): string {
	return [
		...
		"单文件、确定性的小改（改一个常量、替换一个字符串、重命名一个符号）**不要写契约**——直接动手，改动由台账自动入账。",
	].join("\n");
}
...
export function buildImpactDirective(): string {
	return [
		...
		"改动由插件**自动入账**（含内容摘要），不要为每处改动重复记账；只在**真验证跑过之后**用 lume_change 推进状态（一个目标一次即可），并在 change/verify 里补上自动摘要说不清的部分（怎么验的、为什么这么改）。",
	].join("\n");
}
 ...
 */
export function buildRequirementMethodDirective(taskMethods = true): string {
...
export function buildDesignMethodDirective(): string {
	return [
		...
		"纯机械替换、文案级调整、只改常量或字符串的改动**不要写设计决策**——那是一次性动作，不是取舍；写了只是纯开销（A/B 实测里出现过）。",
...
export function buildQuestionAuditDirective(audit: { count: number; unsupported: readonly string[] }): string | null {
	if (audit.count === 0) return null;
	if (audit.count <= MAX_OPEN_QUESTIONS && audit.unsupported.length === 0) return null;
	const lines = [
...
export function buildRequirementCoverageDirective(
	...
	for (const row of rows.slice(0, 10)) {
		...
		if (row.mentions.length === 0) lines.push("   交付物里**没找到**提到这条的句子——先确认是漏了、还是有意剔除（有意就写明归属）。");
