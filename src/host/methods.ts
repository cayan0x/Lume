 ...
 */
export function buildRequirementMethodDirective(): string {
	return [
		"〔需求解读〕用户刚给了或改了需求。下面三条是硬规则：",
		...
		"- 问题预算：先读代码，把代码能回答的答掉；只问代码答不了的，最多 2 个，每个附上你的默认假设与「若你不反对我按这个做」。",
	].join("\n");
}
...
export function buildDriftDirective(words: string[]): string | null {
	...
	return `〔需求漂移〕你的输出里出现了需求原话里没有的变更类型：${words.join("、")}。需求里没有这些——先确认是需求要求的，还是你自己补出来的；如果是脑补的，收回并只按需求做。`;
}
