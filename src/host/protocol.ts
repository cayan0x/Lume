...
/**
 * 机械替换类小改的动词信号（只用于「豁免契约/设计提醒」，不参与路由）。
 *
 * 为什么需要：A/B 实测（2026-09-25）里，连「把三个文件里的域名全部替换掉」这种确定性任务
 * 也照样触发了〔先量化后动手〕并真的写了一份契约——8 条会话里 4 条调了 lume_contract。
 * 契约的价值在**多步/多目标/有歧义**的任务；对"改一个常量/替换一个字符串"它只是纯开销。
 */
export const SMALL_EDIT_RE = /全部替换|统一替换|批量替换|替换成|改成|改为|重命名|改名/;

/**
 * 判据集中在这里（机制只写一遍）：有机械动作信号、**且**没有设计信号，才算小改。
 * 设计信号那道闸很关键——否则「把登录接口改成支持分页」也会被误判成小改而豁免契约。
 */
export function isSmallMechanicalEdit(text: string): boolean {
	return SMALL_EDIT_RE.test(text) && !DESIGN_SIGNAL_RE.test(text);
}
