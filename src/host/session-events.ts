/**
 * 会话事件处理器（从 index.ts 抽出，第 ③ 项拆分）。
 *
 * 为什么单独成模块：这段原本是 index.ts 里 280 行的内联闭包，与提示装配、工具注册、
 * 存储初始化挤在同一个文件；事件语义（记账 / 计数器 / 触发器 / 提醒）与「怎么拼提示词」
 * 是两件事，混在一起时改一处要读半个文件。
 *
 * 依赖通过 `SessionEventDeps` 显式声明（**全部由 index.ts 注入**），所以这里没有隐藏
...
export function createSessionEventHandler(deps: SessionEventDeps) {
	return (session: HostPayload, event: HostPayload): void => {
		...
		switch (event.type) {
			case "request/context": {
				...
				if (typeof data?.provider === "string" && typeof data?.model === "string") {
					...
					deps.ctx.logger?.warn?.(
						`lume: request/context 更新 deps.llmRoute → ${deps.llmRoute.current?.provider}/${deps.llmRoute.current?.model}`,
					);
				} else {
			...
			case "tool/call": {
				st.toolCalls++;
				if (st.interactionMode === "execute") st.taskPhase = deps.advancePhase(st.taskPhase, "execute");
				...
				if (deps.projectMemoryOn && st.toolKind === "mutate") {
					...
					if (target) {
						...
						deps.projectTask(sid, "自动改动入账", (store) =>
							store.upsertChange(sid, {
								target,
					...
					if (st.triggerCounters.mutations === 1 && target && !st.agent.inspectedTargets.has(target)) {
						...
						if (!deps.noticeText(st, "trigger"))
							deps.forceNotice(
								st,
								"trigger",
								`〔先定位〕你要改 ${target}，但本会话还没有读过它——已经摸过的是：${seen}。先打开要改的那段（含调用方与配置/SQL 绑定），确认现有实现再动手；改完立刻回读或跑最小验证。`,
							);
	...
	return (session: HostPayload): void => {
		...
		if (deps.reflectionEnabled && turns.length >= 4) {
			void (async () => {
				...
				deps.ctx.logger?.warn?.(
					`lume: 反思日志 ${sid} context=${score.context} planning=${score.planning} verification=${score.verification} review=${score.review} diagnosis=${score.diagnosis}「${score.note}」`,
				);
			})();
		}
	};
