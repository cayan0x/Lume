...
export function sessionDirSlug(sid: string, dsHome: string): string | null {
...
function mapPath(dsHome: string): string {
	return join(dsHome, "harness", MAP_FILE);
}

export function readWorkspaceMap(dsHome: string): Record<string, string> {
	try {
		const raw = JSON.parse(readFileSync(mapPath(dsHome), "utf8")) as unknown;
...
export function rememberWorkspace(dsHome: string, slug: string | null, cwd: string | null): void {
	if (!dsHome || !slug || !cwd) return;
	const map = readWorkspaceMap(dsHome);
	if (map[slug] === cwd) return;
	map[slug] = cwd;
	try {
...
export function workspaceFromSlug(dsHome: string, slug: string | null): string | null {
	if (!dsHome || !slug) return null;
