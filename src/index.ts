		...
		() =>
			ctx.on("session/event", (session: any, event: any) => {
				...
				switch (event.type) {
					...
					case "tool/call": {
						st.toolCalls++;
						if (st.interactionMode === "execute") st.taskPhase = "execute";
						break;
					}
					case "tool/result": {
					...
					case "turn/end": {
						...
						st.postTurnReview = st.interactionMode === "execute" && st.assistantText && !claimsVerification
							...
							: null;
						if (st.interactionMode === "execute") st.taskPhase = st.toolFailures > 0 || st.toolUnknown > 0 ? "diagnose" : claimsVerification ? "deliver" : "verify";
