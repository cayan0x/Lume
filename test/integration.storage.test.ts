...
it("PersonaStore drives the real storage stack: LRU + persistence layout", async () => {
...
it("IdentityStore drives the real storage stack: schemastery record schemas accept plain objects", async () => {
	const { root, tables } = await openProbeStack();
...
it("domains reopen with existing records: the schemastery schema bridge survives open-time validation", async () => {
	const root = mkdtempSync(join(tmpdir(), "lume-it-"));
	roots.push(root);
	...
	await identity.setCustomPersona("kaguya", {
		...
		promptText: "以「傲娇」性格回应。",
		createdAt: 1,
		corpus: [{ user: "帮我看看这个报错", assistant: "哼，谁让你乱改配置的。……啦，帮你看看还不行嘛。" }],
	});
	...
	expect(persona?.corpus).toEqual([{ user: "帮我看看这个报错", assistant: "哼，谁让你乱改配置的。……啦，帮你看看还不行嘛。" }]);
	expect(new PersonaStore(second.tables.get("session_persona")).get("s1")).toBe("loli");
	await second.close();
}, 20_000);

it("ReflectionStore persists scores against the real storage stack", async () => {
	const root = mkdtempSync(join(tmpdir(), "lume-it-"));
	roots.push(root);
